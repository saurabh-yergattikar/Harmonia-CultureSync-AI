...
async function initializeGeminiSession(apiKey, customPrompt = '', profile = 'interview', language = 'en-US', isReconnection = false) {
    ...
    const qlooApiKey = localStorage.getItem('qlooApiKey') || null;
    ...
    const systemPrompt = isCulturalProfile 
        ? getCulturalSystemPrompt(profile, customPrompt, googleSearchEnabled)
        : getSystemPrompt(profile, customPrompt, googleSearchEnabled);
                ...
                onmessage: function (message) {
                    ...
                    if (message.serverContent?.inputTranscription?.text) {
                        ...
                        if (isCulturalProfile && culturalEngine) {
                            console.log(`🔥 QLOO ANALYSIS: Processing "${message.serverContent.inputTranscription.text}"`);
                            
                            culturalEngine.analyzeConversation(
                                message.serverContent.inputTranscription.text
                            ).then(culturalAnalysis => {
